...
export function RenameFileButton({
  ...
  const [status, setStatus] = useState<'idle' | 'renaming' | 'error'>(
    'idle',
  );
  ...
  async function handleRename(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // Refresh the parent FileBrowser.
      onRenamed?.();

      // Close the rename drawer.
      onOpenChange(false);

      // Navigate to the renamed file.
      router.push(
        `/repos/${owner}/${repo}/edit/${trimmedNew}?branch=${encodeURIComponent(branch)}`,
      );
    } catch {
      setStatus('error');
