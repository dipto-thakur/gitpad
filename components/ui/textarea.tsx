...
export const Textarea = React.forwardRef<HTMLTextAreaElement, React.TextareaHTMLAttributes<HTMLTextAreaElement>>(
  ({ className, ...props }, ref) => (
    <textarea
      ref={ref}
      className={cn(
        'flex w-full border border-border/80 bg-muted/60 px-7 py-3.5 text-[14px] leading-relaxed text-foreground',
        'shadow-[inset_0_1px_2px_rgba(0,0,0,0.02)]',
        'placeholder:text-muted-foreground',
        'transition-colors duration-150',
        'focus:border-zinc-300 focus:bg-zinc-100/90',
        'disabled:cursor-not-allowed disabled:opacity-40',
        'border-border/80 dark:bg-muted/60 dark:focus:border-zinc-700 dark:focus:bg-zinc-900/90',
